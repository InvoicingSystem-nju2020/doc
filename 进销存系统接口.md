...
| supplierName               | String | 是   | 李宁           | 供应商名称                       |
...
| 名称           | 类型   | 必选        | 示例值 | 说明                                       |
| -------------- | ------ | ----------- | ------ | ------------------------------------------ |
| startTime      | String | 否          | 2019/3 | 按进货时间查询时的起始时间                 |
| endTime        | String | 否          | 2019/4 | 按进货时间查询的终止时间                   |
| goodsName      | String | 否          | 羽球拍 | 按商品名查询                               |
| goodsNumber    | String | 否          |        | 按商品编号查询                             |
| brand          | String | 否          | LINING | 按品牌查询                                 |
...
| salesPerson    | String | 是   | 张三        | 销售员                      |
| clientName     | String | 是   | 南京海关    | 客户名                      |
| writeAnInvoice | String | 是   | 1           | 是否开票。0为false，1为true |
| goodsNumber    | String | 是   |             | 商品编号                    |
| brand          | String | 是   | LINING      | 品牌                        |
| goodsName      | String | 是   | 羽球拍      | 商品名                      |
| goodsNo        | String | 是   | WRT74181U2  | 货号                        |
| buyingPrice    | double | 是   | 1240        | 进货价                      |
...
| clientsType   | String | 否          | 团购     | 按客户类型查询                             |
| startIndex    | int    | 否，默认0   | 100      | 从符合条件降序的所有进货记录中第几个开始取 |
| num           | int    | 否，默认100 | 100      | 从开始位置取100个                          |
